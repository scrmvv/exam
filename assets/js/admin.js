...
function renderCounters(applications) {
  ...
  counters.innerHTML = `
    ...
    <div class="course-chip">Новые<br><strong class="text-2xl">${count('Новая')}</strong></div>
    <div class="course-chip">Идет обучение<br><strong class="text-2xl">${count('Идет обучение')}</strong></div>
    <div class="course-chip">Завершено<br><strong class="text-2xl">${count('Обучение завершено')}</strong></div>
  `;
}
...
function render() {
  ...
  const filtered = applications.filter((app) => (
    `${app.userName} ${app.login} ${app.courseName} ${app.status}`.toLowerCase().includes(query)
  ));
...
form?.addEventListener('submit', (event) => {
  ...
  setNotice(result, 'Неверный логин или пароль администратора.');
});
